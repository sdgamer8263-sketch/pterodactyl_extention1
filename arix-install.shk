...
# API Base for License Verification
    ...
    while true; do
        ...
        case $nb_choice in
            1)
                ...
                log_installation "non-blueprint" "v2.1.0"
    ...
    while true; do
        ...
        case $b_choice in
            1)
                ...
                log_installation "blueprint" "v2.1.0-blueprint"
...
# Temporarily disable set -e so the script doesn't silently die here
set +e 

# Check which package manager is available and install dependencies silently
if [ -x "$(command -v apt-get)" ]; then
    ...
    echo -e "${YELLOW}Warning: Could not detect apt, dnf, or yum. Assuming unzip, curl, and wget are installed.${NC}"
fi

# Re-enable set -e for the rest of the critical operations
set -e
