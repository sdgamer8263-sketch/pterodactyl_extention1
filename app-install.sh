        ...
        if [[ "$action_type" == "install" ]]; then
            ...
            for idx in "${selected_addons[@]}"; do
                if [[ "${addon_names[$idx]}" != "resourcemanager.blueprint" ]]; then 
                    ...
                    # === EGG CHANGER SPECIFIC FIX INJECTION ===
...
EOF
                        ...
                        # Frontend me naya component add kiya gaya hai, isliye build karna zaroori hai
                        export NODE_OPTIONS="--openssl-legacy-provider --no-deprecation"
                        ( yarn build:production > /dev/null 2>&1 ) & spinner $! 

                        # Pterodactyl cache clear karna
                        php artisan view:clear > /dev/null 2>&1
                        php artisan optimize:clear > /dev/null 2>&1
                        success "Successfully fixed."
                    fi
                    # === END EGG CHANGER SPECIFIC FIX INJECTION ===
                fi
            done
        else
