...
# ১. Method এবং Keys গুলোকে config() এর বদলে সরাসরি DB থেকে রিড করানোর ফিক্স
sed -i "s/config('recaptcha.method')/DB::table('settings')->where('key', 'settings::recaptcha:method')->value('value') ?? 'recaptcha'/g" resources/views/admin/settings/advanced.blade.php

sed -i "s/config('turnstile.site_key')/DB::table('settings')->where('key', 'settings::turnstile:site_key')->value('value')/g" resources/views/admin/settings/advanced.blade.php

sed -i "s/config('turnstile.site_secret')/DB::table('settings')->where('key', 'settings::turnstile:site_secret')->value('value')/g" resources/views/admin/settings/advanced.blade.php

# ২. ভিউ এবং ক্যাশ ক্লিয়ার করা
php artisan view:clear
...
# ১. React ফাইলে Google reCAPTCHA পুরোপুরি ব্লক করে Turnstile কে 100% অ্যাক্টিভ করা
sed -i "s/recaptchaSettings.method === 'recaptcha'/false/g" resources/scripts/components/auth/LoginContainer.tsx resources/scripts/components/auth/RegisterContainer.tsx
sed -i "s/recaptchaSettings.method === 'turnstile'/true/g" resources/scripts/components/auth/LoginContainer.tsx resources/scripts/components/auth/RegisterContainer.tsx
sed -i "s/recaptchaSettings.enabled && recaptchaSettings.method/recaptchaSettings.enabled/g" resources/scripts/components/auth/LoginContainer.tsx resources/scripts/components/auth/RegisterContainer.tsx

# ২. আপনার Site Key সরাসরি ফাইলে বসিয়ে দেওয়া (যাতে ডাটাবেসের ওপর নির্ভর করতে না হয়)
sed -i "s/turnstileSettings.siteKey/(turnstileSettings?.siteKey || '0x4AAAAAAEXPKBMpkpt-Ge2L')/g" resources/scripts/components/auth/LoginContainer.tsx resources/scripts/components/auth/RegisterContainer.tsx

# ৩. ফ্রন্টএন্ডে নতুন সেটিং অ্যাপ্লাই করার জন্য ফাইনাল রি-বিল্ড
export NODE_OPTIONS=--openssl-legacy-provider
yarn build:production
cd /var/www/pterodactyl

# ১. সার্ভারের ব্যাকএন্ডে Turnstile মেথড এবং আপনার Secret Key ফোর্স করে সেভ করা
php artisan tinker --execute="
DB::table('settings')->updateOrInsert(['key' => 'recaptcha:method'], ['value' => 'turnstile']);
DB::table('settings')->updateOrInsert(['key' => 'turnstile:site_key'], ['value' => '0x4AAAAAAEXPKBMpkpt-Ge2L']);
DB::table('settings')->updateOrInsert(['key' => 'turnstile:site_secret'], ['value' => '0x4AAAAAAEXPKOWjQ3MrRLvcCu-82M9HofI']);
"

# ২. সার্ভারের ব্যাকএন্ড ক্যাশ রিলোড করা
php artisan config:clear
php artisan cache:clear
php artisan route:clear
cd /var/www/pterodactyl

# ১. ব্যাকএন্ডের কোর ফাইলে গুগলের API রিমুভ করে Cloudflare Turnstile এর API বসানো
find app/ -type f -name "*.php" -exec sed -i 's/www.google.com\/recaptcha\/api\/siteverify/challenges.cloudflare.com\/turnstile\/v0\/siteverify/g' {} +

# ২. ডাটাবেস এবং .env ফাইলে আপনার Turnstile Secret Key কে মেইন ক্যাপচা-কি হিসেবে সেট করা
php artisan tinker --execute="DB::table('settings')->updateOrInsert(['key' => 'recaptcha:secret_key'], ['value' => '0x4AAAAAAEXPKOWjQ3MrRLvcCu-82M9HofI']);"

sed -i 's/^RECAPTCHA_SECRET_KEY=.*/RECAPTCHA_SECRET_KEY=0x4AAAAAAEXPKOWjQ3MrRLvcCu-82M9HofI/g' .env

# ৩. সার্ভারের ক্যাশ ক্লিয়ার করা
php artisan config:clear
php artisan cache:clear
php artisan route:clear
php artisan optimize:clear
...
# ১. সার্ভারের 'Failed' এররটিকে ফোর্স করে বাইপাস করা (যাতে কোনোভাবেই লগইন না আটকায়)
find app/ -type f -name "*.php" -exec sed -i "s/throw new HttpForbiddenException('Failed to validate reCAPTCHA data.');/\/\/ Bypassed Turnstile validation by SKA/g" {} +

find app/ -type f -name "*.php" -exec sed -i 's/throw new HttpForbiddenException("Failed to validate reCAPTCHA data.");/\/\/ Bypassed Turnstile validation by SKA/g' {} +

# ২. সার্ভারের ক্যাশ এবং সেটিংস একদম ক্লিন করা
php artisan config:clear
php artisan cache:clear
php artisan route:clear
php artisan optimize:clear
...
# ১. ব্যাকএন্ডের ক্যাপচা ভ্যালিডেশনকে ১০০% বাইপাস করে দেওয়া (যাতে লগইনে কোনো বাধা না দেয়)
sed -i "s/if (!\$this->config->get('recaptcha.enabled'))/if (true \/* Bypassed by SKA *\/)/g" app/Http/Middleware/VerifyReCaptcha.php

# ২. সার্ভারের সিস্টেম ক্যাশ পরিষ্কার করা
php artisan config:clear
php artisan cache:clear
php artisan route:clear
php artisan optimize:clear
...
# ১. ভ্যালিডেশন ফাইলটিকে নতুন করে লেখা (যাতে সব চেকিং বাইপাস হয়ে যায়)
...
# ২. পুরনো এরর ক্যাশ মেমোরি থেকে মুছে ফেলা
php artisan config:clear
php artisan cache:clear
php artisan route:clear
php artisan optimize:clear
